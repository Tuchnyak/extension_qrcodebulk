(() => {
  ...
  function updateCSVControls() {
    ...
    const hasCSVData = textareaContent.split("\n").some(
      (line) => line.trim() && line.includes(separator)
    );
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const fullMessage = errorCount > 0 ? `${message} ${errorCount} lines had errors. See errors.log for details.` : message;
  ...
  async function generateAndDownloadQRCode(lineData, imageSize, includeTopText, includeBottomText, filePath) {
    return new Promise((resolve, reject) => {
      import_qrcode.default.toCanvas(lineData.url, { width: imageSize }, (error, qrCanvas) => {
        if (error) {
  ...
  async function createErrorLog(errors, subDir) {
    const errorContent = errors.map(
      (error) => `Line ${error.lineNumber}: ${error.line} - ${error.reason}`
    ).join("\n");
